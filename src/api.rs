...
extension_sql!(
    ...
    -- Statement-level trigger for INSERT: fires ONCE per statement,
    -- bulk INSERT...SELECT from transition table instead of 100K per-row calls.
    ...
    BEGIN
        ...
        END IF;
        v_max_size   := COALESCE(pg_size_bytes(current_setting('pg_flashback.max_row_size', true)), 8192);
    ...
    -- Per-row trigger for UPDATE: old/new correlation needs row-level access.
    ...
    BEGIN
        ...
        IF pg_column_size(NEW) > v_max_size OR pg_column_size(OLD) > v_max_size THEN
            RAISE WARNING 'pg_flashback: row too large, skipping capture for %',
                          v_table_name;
            RETURN NULL;
        END IF;
    ...
    -- Statement-level trigger for DELETE: same bulk approach as INSERT.
    ...
    BEGIN
        ...
        -- Drop new triggers if re-tracking
        ...
        -- Statement-level INSERT trigger with transition table
        EXECUTE format(
        ...
        -- Per-row UPDATE trigger (old/new correlation)
        EXECUTE format(
        ...
        -- Statement-level DELETE trigger with transition table
        EXECUTE format(
    ...
    BEGIN
        ...
        SELECT tracked_since
          INTO v_tracked_since
    ...
    -- Show retention status per tracked table, with WARNING flag for tables
    -- where oldest restorable point is within 10% of retention window.
    ...
    BEGIN
        ...
        LOOP
            ...
            SELECT rec.tbl,
                   ...
                   -- Warning when >90% of retention used
                   COALESCE(
                       (clock_timestamp() - min(d.event_time)) > (rec.ri * 0.9),
                       false
                   )
        ...
        IF v_rel_oid IS NULL THEN
            ...
            FROM flashback.delta_log d
                        WHERE d.committed_at IS NOT NULL
                            AND (
                                    d.table_name = target_table
               OR d.table_name = format('public.%s', target_table)
                            )
    ...
    BEGIN
        ...
        ELSE
              ...
              INTO tracked
            FROM flashback.tracked_tables tt
            WHERE tt.schema_name = input_schema
              AND tt.table_name = input_table
              AND tt.is_active
            LIMIT 1;
        END IF;
