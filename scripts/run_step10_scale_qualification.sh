...
insert_batch() {
    ...
    case "$shape" in
      mixed)
        s10_q "INSERT INTO $rel(id,account_no,amount,ratio,created_at,is_active,label,notes)
...
writer_insert_sql() {
    ...
    case "$shape" in
      mixed)
        echo "INSERT INTO $rel(id,account_no,amount,ratio,created_at,is_active,label,notes)
              VALUES ($id, $(( id % 100000 )), 1.25, 0.5, now(), true, '$tag-lbl-$id', '$tag-note-$id');"
        ;;
      toast)
...
run_tier() {
    ...
    (( protect_rc == 0 )) || { log "  protect failed; see $protect_log"; sed -n '1,40p' "$protect_log" >&2; }
