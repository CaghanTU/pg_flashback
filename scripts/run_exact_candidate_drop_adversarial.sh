...
# Expression indexes are rejected by the local compatibility contract.
q "CREATE TABLE public.adv_expr(id int PRIMARY KEY, payload jsonb NOT NULL);
    CREATE INDEX adv_expr_payload_idx ON public.adv_expr ((payload->>'k'));"
rc=0
q "SELECT flashback_track('public.adv_expr');" >/tmp/pgfb-adv-expr.err 2>&1 || rc=$?
[[ "$rc" != 0 ]] || fail_case "expression_indexes_rejected" "track accepted expression index"
grep -Eiq 'expression_indexes|not compatible' /tmp/pgfb-adv-expr.err \
    || fail_case "expression_indexes_rejected" "unexpected error text"
q "DROP TABLE public.adv_expr;"
pass "expression_indexes_rejected"

# Different schema DROP + recover via CLI (supported local shapes only).
q "CREATE SCHEMA sales;
    ...
    CREATE INDEX orders_note_idx ON sales.orders (note);
