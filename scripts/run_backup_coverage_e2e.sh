...
# FULL-only activate after marker, prepare admission, and legacy coverage
# rejection. Full helper swap E2E remains scripts/run_recovery_helper_e2e.sh.
...
if q "SELECT flashback_activate_backup_anchor(
        'public.backup_cov', 'repo', 'stanza', 'overlapF',
        $SYSID, $TIMELINE, 'manifest-overlap', repeat('aa', 32),
        '$MARKER'::pg_lsn, '$STOP'::pg_lsn);" 2>/dev/null; then
  echo "FAIL: overlapping FULL start was accepted"
  exit 1
fi

GEN=$(q "SELECT flashback_activate_backup_anchor(
        'public.backup_cov', 'repo', 'stanza', '20260717E2EF',
...
echo "ok: backup coverage track/resolve/activate/prepare"
