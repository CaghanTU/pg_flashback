...
# Per-run measurements belong only in the ignored result artifact. Mutating a
# tracked design document here would dirty the exact source tree and invalidate
# any qualification gate that follows this PoC in the same checkout.
log "run evidence recorded only in $RESULT_JSON"
